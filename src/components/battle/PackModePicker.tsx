...
const OPTIONS: { value: PackMode; label: string }[] = [
  ...
  { value: "tripletagteamsir", label: "Triple Tag Team SIRs" },
];
...
export default function PackModePicker({ value, onChange }: Props) {
  return (
    <div className="mt-4">
        ...
        {OPTIONS.map((opt, i) => (
          <button
            ...
            className={`rounded-xl border py-2.5 text-xs font-bold transition-all duration-200 active:scale-95 ${
              i === OPTIONS.length - 1 && OPTIONS.length % 2 === 1 ? "col-span-2" : ""
            } ${
              value === opt.value
      ...
      {value === "tripletagteamsir" && (
        <p className="mt-1.5 text-center text-[11px] leading-relaxed text-slate-500">
          Every pack is a Special Illustration Rare Triple Tag Team - art type and special form are
          both locked in for everyone.
        </p>
      )}
    </div>
  );
