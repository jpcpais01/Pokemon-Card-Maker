...
export default function VotingPanel({ images, myVote, votedCount, totalVoters, busy, onVote }: Props) {
  ...
  return (
    <div>
      <p className="mb-4 text-center text-[12.5px] leading-relaxed text-slate-400">
        Vote for your favorite — not your own. Tap a selected card again to see it full size.
      </p>
        ...
        {images.map((image, slot) => {
          ...
          return (
            <button
              ...
              className={`overflow-hidden rounded-2xl border transition-all duration-200 active:scale-[0.97] disabled:active:scale-100 ${
                isActive
                  ? "border-amber-300/70 shadow-[0_0_26px_-6px_rgba(255,201,75,0.6)]"
                  : "border-white/10"
              }`}
            >
              <div className="relative aspect-[3/4] w-full bg-black/40">
                ...
                {hasVoted && isActive && (
                  <span className="absolute left-1.5 top-1.5 rounded-full bg-amber-400 px-2 py-0.5 text-[9px] font-black tracking-wide text-[#2a1705]">
                    YOUR VOTE
                  </span>
                )}
              ...
              <div className="bg-slate-900/90 px-2 py-1.5 text-center">
                <p className="text-[11.5px] font-bold text-white">Card {slot + 1}</p>
              </div>
            </button>
          );
        })}
