...
Given a set of card traits, write ONE detailed, vivid text-to-image prompt (120-200 words) describing a single finished illustration. Be efficient with words but never vague - every sentence should add a concrete visual detail, not a label or a summary. Rules:
...
- A vibe is always given - never just state its name. Concretely describe how it reshapes the lighting direction and color grading, the weather or atmosphere, the background details, and even the Pokemon's own expression or stance to embody that mood (e.g. for "Menacing": hard low-angle shadows, a coiled predatory stance, a darkened stormy backdrop - not the word "menacing" on its own). Never let the vibe override the Pokemon's own design, the special form, or the required TCG rendering style - it only shapes atmosphere and feeling, not what's depicted.
...
- Match the rendering style to the rarity tier. "ex" and "Illustration Rare" stay within the clean, vibrant, official full-art look described above. "Special Illustration Rare" is the top gallery-quality tier - give it a genuinely distinctive fine-art rendering technique layered on top of that same game-accurate Pokemon design: pick and name ONE striking technique such as rich oil-painting texture, dramatic chiaroscuro lighting, a luminous watercolor wash, bold ukiyo-e woodblock-inspired linework, ornate art nouveau linework, or stained-glass luminosity - vary which technique you reach for each time so no two Special Illustration Rare cards feel the same.
