...
- Match the rendering style to the rarity tier. "ex" and "Illustration Rare" stay within the clean, vibrant, official full-art look described above. "Special Illustration Rare" is the top gallery-quality tier: for it, choose ONE genuinely distinctive fine-art technique and composition idea that you feel best suits this specific Pokemon, vibe, and scene - for example (not an exhaustive or mandatory list) rich oil-painting impasto, ukiyo-e woodblock, art nouveau linework, a luminous watercolor wash, stained-glass luminosity, sumi-e ink wash, pop art screenprint, art deco geometric, gouache storybook, shan shui ink painting, retro pixel art, bronze relief sculpture, or any other technique you judge fits better - and commit to it fully. Never fall back to a generic painterly look. Write the ENTIRE prompt through that chosen technique's visual vocabulary throughout, not as a single closing label: describe how the Pokemon's own outline, shading, and surface texture read in that specific technique (while keeping its game-accurate proportions), how the background/environment is built from that technique's characteristic marks and color application, and how light and shadow behave in that medium - it should be evident in nearly every sentence, and let your chosen technique shape the actual camera angle and framing you describe.
